...
public class UMSettingEditor : Editor {
	...
	public override void OnInspectorGUI() {
		...
		case 4:
			bool oldState = SA.Analytics.Google.GA_Settings.Instance.AutoCampaignTracking;
			SA.Analytics.Google.SettingsEditor.DrawAnalyticsSettings();

			if (oldState != SA.Analytics.Google.GA_Settings.Instance.AutoCampaignTracking) {
				...
				SA.Manifest.ApplicationTemplate app = manifest.ApplicationTemplate;
				SA.Manifest.PropertyTemplate service = app.GetOrCreatePropertyWithName("service", "com.google.android.gms.analytics.CampaignTrackingService");
				SA.Manifest.PropertyTemplate receiver = app.GetOrCreatePropertyWithName("receiver", "com.androidnative.analytics.ReferalIntentReciever");

				if (SA.Analytics.Google.GA_Settings.Instance.AutoCampaignTracking) {
	...
	public static bool IsInstalled {
		get {
			return SA.Common.Editor.VersionsManager.Is_UM_Installed;
		}
	}

	public static bool IsUpToDate {
		get {
			if(CurrentVersion == SA.Common.Editor.VersionsManager.UM_Version) {
				return true;
			} else {
				return false;
			}
	...
	public static int CurrentVersion {
		get {
			return SA.Common.Editor.VersionsManager.ParceVersion(UltimateMobileSettings.VERSION_NUMBER);
		}
	}
	
	public static int CurrentMagorVersion {
		get {
			return SA.Common.Editor.VersionsManager.ParceMagorVersion(UltimateMobileSettings.VERSION_NUMBER);
		}
	}
	...
	private void InstallOptions() {
		...
		if(IsInstalled) {
			if(!IsUpToDate) {
				EditorGUILayout.BeginVertical (GUI.skin.box);
				EditorGUILayout.HelpBox("Update Required \nResources version: " + SA.Common.Editor.VersionsManager.UM_StringVersionId + " Plugin version: " + UltimateMobileSettings.VERSION_NUMBER, MessageType.Warning);
				...
				if(CurrentMagorVersion != SA.Common.Editor.VersionsManager.UM_MagorVersion) {
	...
	public static void UpdateGoogleAdIOSAPI(bool forseDisable = false) {
		...
		string IOSADBannerContent = SA.Common.Util.Files.Read(SA.Common.Config.MODULS_PATH + "GoogleMobileAd/Scripts/IOS/IOSADBanner.cs");
		string IOSAdMobControllerContent = SA.Common.Util.Files.Read(SA.Common.Config.MODULS_PATH + "GoogleMobileAd/Scripts/IOS/IOSAdMobController.cs");
		string GoogleMobileAdPostProcessContent = SA.Common.Util.Files.Read(SA.Common.Config.MODULS_PATH + "GoogleMobileAd/Scripts/Editor/GoogleMobileAdPostProcess.cs");
		string ScarletPostProcessorContent = SA.Common.Util.Files.Read(SA.Common.Config.MODULS_PATH + "GoogleMobileAd/Scripts/Editor/ScarletPostProcessor.cs");
		...
		if(IsGoogAdIOSEnabled)  {
			...
			SA.Common.Editor.Instalation.InstallGMAPart();
			...
			SA.Common.Util.Files.DeleteFile(SA.Common.Config.IOS_DESTANATION_PATH + "GMA_SA_Lib_Proxy.mm");
			SA.Common.Util.Files.DeleteFile(SA.Common.Config.IOS_DESTANATION_PATH + "GMA_SA_Lib.h");
			SA.Common.Util.Files.DeleteFile(SA.Common.Config.IOS_DESTANATION_PATH + "GMA_SA_Lib.m");
		}
		
		SA.Common.Util.Files.Write(SA.Common.Config.MODULS_PATH + "GoogleMobileAd/Scripts/IOS/IOSADBanner.cs", IOSADBannerContent);
		SA.Common.Util.Files.Write(SA.Common.Config.MODULS_PATH + "GoogleMobileAd/Scripts/IOS/IOSAdMobController.cs", IOSAdMobControllerContent);
		SA.Common.Util.Files.Write(SA.Common.Config.MODULS_PATH + "GoogleMobileAd/Scripts/Editor/GoogleMobileAdPostProcess.cs", GoogleMobileAdPostProcessContent);
		SA.Common.Util.Files.Write(SA.Common.Config.MODULS_PATH + "GoogleMobileAd/Scripts/Editor/ScarletPostProcessor.cs", ScarletPostProcessorContent);
	...
	private void InAppSettings() {
			...
			if(settings.IsInAppSettingsProductsOpen) {
				...
				foreach(UM_InAppProduct p in settings.InAppProducts) {
					...
					if(p.IsOpen) {
						...
						p.id = EditorGUILayout.TextField(InAppID, p.id);
						p.DisplayName = EditorGUILayout.TextField(DisplayNameLabel, p.DisplayName);
						...
						EditorGUI.indentLevel++; {
							...
							p.IOSId = EditorGUILayout.TextField(IOSSKU, p.IOSId);
							EditorGUILayout.TextField("TvOS SKU[?]:", p.IOSId);
							p.AndroidId = EditorGUILayout.TextField(AndroidSKU, p.AndroidId);
							p.WP8Id = EditorGUILayout.TextField(WP8SKU, p.WP8Id);
		...
		SA.Common.Editor.Tools.BlockHeader("Testing Settings");


		UltimateMobileSettings.Instance.Is_InApps_EditorTestingEnabled = SA.Common.Editor.Tools.ToggleFiled("Editor Testing", UltimateMobileSettings.Instance.Is_InApps_EditorTestingEnabled);
	...
	public static void UpdatePluginSettings() {
		...
		SA.Common.Editor.Tools.ChnageDefineState(UM_InAppPurchaseManagerPath, "ATC_SUPPORT_ENABLED", AndroidNativeSettings.Instance.EnableATCSupport);
	...
	private void AboutGUI() {
		...
		SA.Common.Editor.Tools.SelectableLabelField(SdkVersion,   UltimateMobileSettings.VERSION_NUMBER);
		SA.Common.Editor.Tools.SupportMail();
		EditorGUILayout.Space();


		EditorGUILayout.BeginHorizontal();
		SA.Common.Editor.Tools.SelectableLabelField(new GUIContent("Android Native") ,   AndroidNativeSettings.VERSION_NUMBER);
		SA.Common.Editor.Tools.SelectableLabelField(new GUIContent("Google Play Lib"),  AndroidNativeSettings.GOOGLE_PLAY_SDK_VERSION_NUMBER);
